...
export interface CorpusRelationship {
  ...
  relationshipType: 'is-a' | 'part-of' | 'related-to' | 'requires' | 'example-of';
  evidence: string;
  chunks: string[]; // IDs de chunks donde aparece
  sourceFiles: string[];
  confidence: number;
...
export class CorpusBuilder {
   ...
   */
  async addFileContent(fileName: string, markdownContent: string): Promise<void> {
  ...
  async buildCorpus(): Promise<SubjectCorpus> {
    ...
    this.corpus.normalizedMarkdown = this.normalizeMarkdown(this.corpus.rawMarkdown);
...
---\n\n`;
  ...
  private async extractGlobalConcepts(): Promise<CorpusConcept[]> {
    ...
    for (const chunk of this.corpus.chunks) {
      ...
      const definitionPatterns = ['es un', 'se define como', 'significa que', 'consiste en'];
  ...
  private async extractGlobalDefinitions(): Promise<CorpusDefinition[]> {
    ...
    for (const chunk of this.corpus.chunks) {
      const definitionPatterns = [
        { pattern: /(.*?)\s*(es un|se define como|significa que|consiste en)\s*(.+?)(?=\.|\n|$)/i, groups: [1, 3] },
        { pattern: /(.*?):\s*(.+?)(?=\.|\n|$)/, groups: [1, 2] }
      ];
      ...
      for (const { pattern, groups } of definitionPatterns) {
        ...
        for (const match of matches) {
          if (match[groups[0]] && match[groups[1]]) {
            ...
            const concept = this.corpus.concepts.find(c => 
            ...
            if (concept) {
              ...
              const existingDef = definitions.find(d => d.conceptId === concept.id);
        ...
        for (const pattern of relPattern.patterns) {
          ...
          for (const match of matches) {
            if (match[1] && match[2]) {
              ...
              // Buscar conceptos
              const sourceConcept = this.corpus.concepts.find(c => 
                c.normalizedName === sourceConceptName
              );
              const targetConcept = this.corpus.concepts.find(c => 
                c.normalizedName === targetConceptName
              );
              ...
              if (sourceConcept && targetConcept) {
                ...
                if (existingRel) {
                  ...
                  relationships.push({
                    ...
                    sourceConceptId: relPattern.reverse ? targetConcept.id : sourceConcept.id,
                    targetConceptId: relPattern.reverse ? sourceConcept.id : targetConcept.id,
  ...
  private calculateMetrics(): void {
    ...
    const maxOccurrences = Math.max(...this.corpus.concepts.map(c => c.occurrences), 1);
