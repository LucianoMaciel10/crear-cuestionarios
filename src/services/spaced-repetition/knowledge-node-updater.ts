...
import type { IKnowledgeNode, SpacedRepetitionReview } from "../../data/models/knowledge-node.model";
...
export function updateKnowledgeNodeWithSM2Review(
  ...
  const currentSM2State: SM2State = {
    ...
    repetitionCount: node.metadata.spacedRepetition.currentState.repetitionCount
  };
...
export function isKnowledgeNodeDueForReview(node: IKnowledgeNode): boolean {
  ...
  const today = new Date();
  const nextReviewDate = node.metadata.spacedRepetition.currentState.nextReviewDate;
 ...
 */
export function getReviewHistory(node: IKnowledgeNode): SpacedRepetitionReview[] {
...
export function calculateSpacedRepetitionStats(node: IKnowledgeNode) {
  ...
  const averageQuality = history.reduce((sum, r) => sum + r.quality, 0) / totalReviews;
